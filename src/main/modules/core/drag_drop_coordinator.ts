...
import {
  ...
  DragShelfEvent as StateMachineEvent,
    ...
    await this.shelfCreationMutex.runExclusive(async () => {
      try {
        ...
        if (this.shelfCreatedForCurrentDrag && this.dragSessionShelfId) {
          this.logger.info(
            `🔄 Shelf already created for this drag session: ${this.dragSessionShelfId}`
          );
        ...
        if (context.activeShelfId && !this.shelfCreatedForCurrentDrag) {
          ...
          if (shelfConfig && shelfConfig.isVisible) {
            this.logger.info(
              `♻️ Reusing existing shelf from previous session: ${context.activeShelfId}`
            );
  ...
  private handleDragEnd(): void {
    ...
    if (this.shelfCreatedForCurrentDrag && this.dragSessionShelfId) {
      ...
      if (shelfConfig && shelfConfig.items.length === 0) {
        this.logger.info(
          `📦 Shelf ${this.dragSessionShelfId} created but empty - will be auto-hidden`
        );
