...
export class ShelfLifecycleManager extends EventEmitter {
  ...
  public async destroyShelf(shelfId: string): Promise<boolean> {
    try {
      ...
      if (success) {
        this.logger.info(`✅ Shelf destroyed: ${shelfId}`);
      }
      return success;
    } catch (error) {
  ...
  public scheduleEmptyShelfAutoHide(shelfId: string, customTimeout?: number): void {
    ...
    if (!preferences.shelf.autoHideEmpty) {
      this.logger.info(`🚫 Auto-hide disabled - skipping for shelf: ${shelfId}`);
      return;
    }
    ...
    this.timerManager.setTimeout(
      ...
      () => {
        ...
        const context = this.stateMachine.getContext();
        if (context.isDragging) {
          this.logger.info(`🖱️ Shelf ${shelfId} auto-hide blocked - drag in progress`);
        ...
        if (this.shelvesCreatedDuringDrag.has(shelfId)) {
          this.logger.info(`🚫 Shelf ${shelfId} created during drag - deferring auto-hide`);
        ...
        if (this.activeDropOperations.has(shelfId)) {
          this.logger.info(`⏸️ Shelf ${shelfId} is receiving drops - rescheduling`);
  ...
  public async clearEmptyShelves(): Promise<void> {
    ...
    if (emptyShelfIds.length > 0) {
      this.logger.info(`🧹 Clearing ${emptyShelfIds.length} empty shelves`);
      ...
      this.logger.info('✨ No empty shelves to clear');
    }
  }
  ...
  public reevaluateEmptyShelvesForAutoHide(): void {
    const preferences = this.preferencesManager.getPreferences();
    if (!preferences.shelf.autoHideEmpty) {
      this.logger.info('🚫 Auto-hide disabled - skipping re-evaluation');
      return;
    }

    this.logger.info('🔄 Re-evaluating empty shelves for auto-hide');
  ...
  public markShelfReceivingDrop(shelfId: string): void {
    ...
    this.logger.info(`🎯 Shelf ${shelfId} marked as receiving drop`);
  }
  ...
  public markDropComplete(shelfId: string): void {
    ...
    this.logger.info(`✅ Drop operation complete for shelf ${shelfId}`);
  ...
  public clearAllDropOperations(): void {
    ...
    if (count > 0) {
      this.logger.info(`🧹 Clearing ${count} active drop operations`);
  ...
  public handleDragEnd(): void {
    this.logger.info(`🏁 Processing drag end for ${this.shelvesCreatedDuringDrag.size} shelves`);

    // Check each shelf created during drag
    for (const shelfId of this.shelvesCreatedDuringDrag) {
      const config = this.shelfManager.getShelfConfig(shelfId);
      if (config && config.items.length === 0 && !config.isPinned) {
        this.logger.info(`📦 Shelf ${shelfId} is empty after drag - scheduling auto-hide`);
  ...
  public async destroyAllShelves(): Promise<void> {
    ...
    this.logger.info(`🧹 Destroying ${shelfIds.length} active shelves`);
