...
export class ErrorHandler extends EventEmitter {
  ...
  private initializeLogging(): void {
    ...
    try {
      ...
      this.logStream.on('error', _error => {
        // Can't use logger here as it might cause infinite recursion
        // This is a last-resort fallback when file logging fails
      });

      // Rotate log if it's too large
      this.rotateLogIfNeeded(logFilePath);
    } catch (error) {
      // Can't use logger here as it's during initialization
      // This is a critical bootstrap error
    }
  }
  ...
  private cleanOldLogs(): void {
    ...
    try {
      ...
      // Use logger if available, otherwise this is non-critical
      if (logger) {
  ...
  private handleNativeError(error: ErrorReport): void {
    logger.error('Native module error:', error);
  ...
  private handlePerformanceError(error: ErrorReport): void {
    logger.warn('Performance issue detected:', error);
  ...
  private reportError(_error: ErrorReport): void {
