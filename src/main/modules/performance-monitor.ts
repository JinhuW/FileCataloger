...
export class PerformanceMonitor extends EventEmitter {
  ...
  private emitWarning(type: string, value: number): void {
    const warningType = type.includes('cpu') ? 'cpu' : 
                       type.includes('memory') ? 'memory' : 'appMemory';
    ...
    if (!this.warningStates[warningType as keyof typeof this.warningStates]) {
      ...
      errorHandler.handleError(
        `Performance warning: ${type} at ${value.toFixed(1)}%`,
        {
  ...
  private getThresholdForType(type: string): number {
    switch (type) {
      case 'cpu-warning': return this.thresholds.cpuWarning;
      case 'cpu-critical': return this.thresholds.cpuCritical;
      case 'memory-warning': return this.thresholds.memoryWarning;
      case 'memory-critical': return this.thresholds.memoryCritical;
      case 'app-memory-limit': return this.thresholds.appMemoryLimit;
      default: return 0;
    }
  }
  ...
  public getAverageMetrics(duration: number = 60000): Partial<PerformanceMetrics> {
    ...
    const avgAppMemory = relevantMetrics.reduce((sum, m) => sum + m.memory.appUsage, 0) / relevantMetrics.length;
  ...
  public isHealthy(): boolean {
    ...
    return metrics.cpu.usage < this.thresholds.cpuWarning &&
