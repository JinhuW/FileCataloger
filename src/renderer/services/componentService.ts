 ...
 */
export function createValidatedComponent(
  type: ComponentType,
  name: string,
  config?: Partial<TextConfig | SelectConfig | DateConfig | NumberConfig>,
  existingComponents: ComponentDefinition[] = []
): { component: ComponentDefinition | null; validation: ComponentValidationResult } {
  const component = ComponentService.createComponent(type, name, config);
  const validation = ComponentService.validateComponent(component, existingComponents);

  if (!validation.isValid) {
    return { component: null, validation };
  }
...
/**
 * Validate and clone component
 */
export function cloneValidatedComponent(
  definition: ComponentDefinition,
  newName: string,
  existingComponents: ComponentDefinition[] = []
): { component: ComponentDefinition | null; validation: ComponentValidationResult } {
  const cloned = ComponentService.cloneComponent(definition, newName);
  const validation = ComponentService.validateComponent(cloned, existingComponents);

  if (!validation.isValid) {
    return { component: null, validation };
  }

  return { component: cloned, validation };
}
