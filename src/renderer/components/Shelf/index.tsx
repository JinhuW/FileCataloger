...
export const Shelf = React.memo<ShelfProps>(
  ({ config, onConfigChange, onItemAdd, onItemRemove, onClose }) => {
    ...
    useEffect(() => {
      ...
      const itemHeight =
        itemCount > SHELF_CONSTANTS.COMPACT_MODE_THRESHOLD
          ? SHELF_CONSTANTS.ITEM_HEIGHT_COMPACT
          : SHELF_CONSTANTS.ITEM_HEIGHT_NORMAL;
      const itemMargin =
        itemCount > SHELF_CONSTANTS.COMPACT_MODE_THRESHOLD
          ? SHELF_CONSTANTS.ITEM_MARGIN_COMPACT
          : SHELF_CONSTANTS.ITEM_MARGIN_NORMAL;
      ...
      if (itemCount === 0) {
        ...
        setShelfHeight(
          Math.min(Math.max(idealHeight, SHELF_CONSTANTS.MIN_HEIGHT), SHELF_CONSTANTS.MAX_HEIGHT)
        );
      }
    }, [config.items.length]);
    ...
    return (
      <ErrorBoundary>
        <motion.div
          ...
          initial={{
            opacity: ANIMATION_CONSTANTS.SHELF_INITIAL_OPACITY,
            scale: ANIMATION_CONSTANTS.SHELF_INITIAL_SCALE,
          }}
          animate={{
          ...
          onDrop={e => {
            ...
            if (dataTransfer) {
              ...
              if (items.length > 0) {
                ...
                logger.debug('Checking window.api:', !!window.api);
                logger.debug(
                  'Window properties:',
                  Object.keys(window).filter(k => k.includes('api') || k.includes('electron'))
                );
                logger.debug('Full window.api:', window.api);
                logger.debug('Current shelf ID:', config.id);
                ...
                if (window.api && items.length > 0) {
                  ...
                  logger.debug('Found', filePaths.length, 'file paths:', filePaths);
                  ...
                  if (filePaths.length > 0) {
                    ...
                    logger.warn('No file paths found in dropped items');
                  }
                } else {
                  logger.error(
                    'Cannot notify main process: window.api =',
                    !!window.api,
                    ', items =',
                    items.length
                  );
