/**
 * @file mouse_tracker_win32.cpp
 * @brief Windows native mouse tracker using SetWindowsHookEx
 *
 * This module implements high-performance, system-wide mouse tracking for Windows
 * using low-level mouse hooks. It provides real-time mouse position and button
 * state tracking with minimal CPU overhead.
 *
 * Architecture:
 * - SetWindowsHookEx for intercepting mouse events at the system level
 * - Separate thread for message pump processing
...
static WindowsMouseTracker* g_tracker = nullptr;
...
public:
    ...
    bool Start() {
        ...
        try {
            // Start hook thread with message pump
...
private:
    void RunHookThread() {
        ...
        if (!mouse_hook_) {
            ...
            SetError(FileCataloger::ErrorCode::HOOK_CREATE_FAILED,
                    "Failed to create mouse hook. Error code: " + std::to_string(error));
            running_ = false;
            return;
        }
    ...
    static LRESULT CALLBACK LowLevelMouseProc(int nCode, WPARAM wParam, LPARAM lParam) {
        if (nCode >= 0 && g_tracker) {
            MSLLHOOKSTRUCT* hookData = reinterpret_cast<MSLLHOOKSTRUCT*>(lParam);

            g_tracker->events_processed_.fetch_add(1, std::memory_order_relaxed);

            bool button_state_changed = false;
            bool left_button = g_tracker->left_button_down_.load(std::memory_order_relaxed);
            bool right_button = g_tracker->right_button_down_.load(std::memory_order_relaxed);

            switch (wParam) {
                case WM_LBUTTONDOWN:
                    left_button = true;
                    g_tracker->left_button_down_.store(true, std::memory_order_relaxed);
                    button_state_changed = true;
                    break;
                case WM_LBUTTONUP:
                    left_button = false;
                    g_tracker->left_button_down_.store(false, std::memory_order_relaxed);
                    button_state_changed = true;
                    break;
                case WM_RBUTTONDOWN:
                    right_button = true;
                    g_tracker->right_button_down_.store(true, std::memory_order_relaxed);
                    button_state_changed = true;
                    break;
                case WM_RBUTTONUP:
                    right_button = false;
                    g_tracker->right_button_down_.store(false, std::memory_order_relaxed);
                    button_state_changed = true;
                    break;
                case WM_MOUSEMOVE:
            ...
            g_tracker->QueueMouseEvent(
                static_cast<double>(hookData->pt.x),
                static_cast<double>(hookData->pt.y),
                left_button,
                right_button,
            ...
            if (button_state_changed) {
                g_tracker->QueueButtonEvent(left_button, right_button);
            }
        }
...
NAPI_MODULE(mouse_tracker_win32, Init)
