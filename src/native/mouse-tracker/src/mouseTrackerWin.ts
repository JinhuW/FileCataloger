 ...
 *
 * This module provides optimized mouse tracking for Windows using low-level
 * mouse hooks (WH_MOUSE_LL) with:
 ...
 *
 * Features:
 * - 60fps mouse position tracking via SetWindowsHookEx
 ...
 * Requirements:
 * - Windows 10+
 * - Native module built with Windows SDK
 *
...
import { NativeErrorCode } from '@shared/nativeErrorCodes';
...
try {
  ...
  try {
    ...
    nativeModule = require('../build/Release/mouse_tracker_win.node');
  } catch {
    try {
      // Production: from dist/main (webpack output)
      nativeModule = require('./mouse_tracker_win.node');
    } catch {
      // Last resort: absolute path from __dirname
      nativeModule = require(path.join(__dirname, 'mouse_tracker_win.node'));
    }
  }
  logger.info('Successfully loaded Windows native mouse tracker module');
} catch (error: unknown) {
  logger.error('Failed to load Windows native mouse tracker module:', error);
  throw new Error(
    `Failed to load Windows native mouse tracker module: ${error instanceof Error ? error.message : 'Unknown error'}`
  );
}

/**
 * High-performance Windows mouse tracker with event batching
 */
export class WindowsMouseTracker extends EventEmitter implements MouseTracker {
  ...
  private initializeNativeTracker(): void {
    try {
      ...
      logger.debug('Created WindowsMouseTracker instance');
      ...
      logger.info('Windows native tracker initialized successfully');
    } catch (error) {
      logger.error('Failed to initialize Windows native tracker:', error);
      throw new Error(`Failed to initialize Windows native tracker: ${error}`);
    }
  }
  ...
  public start(): void {
    ...
    try {
      ...
      if (success) {
        this.isActive = true;
        logger.info('Windows mouse tracking started');

        // Health check
        setTimeout(() => {
          if (this.eventCount === 0) {
            logger.warn('HEALTH CHECK FAILED: No mouse events received within 2 seconds');
          } else {
            logger.info(`HEALTH CHECK PASSED: ${this.eventCount} mouse events received`);
          }
        }, 2000);
      } else {
